import collections
import pickle
import random
import numpy as np
import matplotlib.pyplot as plt
import torch
from torch.autograd import Variable
import torch.backends.cudnn as cudnn
import torch.nn as nn
import torch.optim as optim
import math
import torch.nn.functional as F
import torch.optim as optim
from numpy import linalg as LA
from tqdm.notebook import tqdm

use_gpu = torch.cuda.is_available()


# ========================================
#      loading datas


def centerDatas(datas):
    datas[:, :n_lsamples] = datas[:, :n_lsamples, :] - datas[:, :n_lsamples].mean(1, keepdim=True)
    datas[:, :n_lsamples] = datas[:, :n_lsamples, :] / torch.norm(datas[:, :n_lsamples, :], 2, 2)[:, :, None]
    datas[:, n_lsamples:] = datas[:, n_lsamples:, :] - datas[:, n_lsamples:].mean(1, keepdim=True)
    datas[:, n_lsamples:] = datas[:, n_lsamples:, :] / torch.norm(datas[:, n_lsamples:, :], 2, 2)[:, :, None]

    return datas


def scaleEachUnitaryDatas(datas):
    norms = datas.norm(dim=2, keepdim=True)
    # print(norms.shape)
    # print(datas / norms)
    return datas / norms


def QRreduction(datas):
    ndatas = torch.qr(datas.permute(0, 2, 1)).R
    ndatas = ndatas.permute(0, 2, 1)
    return ndatas


class Model:
    def __init__(self, n_ways):
        self.n_ways = n_ways


# ---------  GaussianModel
class GaussianModel(Model):
    def __init__(self, n_ways, lamda):
        super(GaussianModel, self).__init__(n_ways)
        self.mus = None  # shape [n_runs][n_ways][n_nfeat]
        self.lamda = lamda

    def clone(self):
        other = GaussianModel(self.n_ways)
        other.mus = self.mus.clone()
        return self

    def cuda(self):
        self.mus = self.mus.cuda()

    def initFromLabelledDatas(self):
        self.mus = ndatas.reshape(n_runs, n_shot + n_queries, n_ways, n_nfeat)[:, :n_shot, ].mean(1)
        # print(ndatas.reshape(n_runs, n_shot + n_queries, n_ways, n_nfeat)[:, :n_shot, ])
        # print(self.mus)
        # 横向求均值后降维了 4->3

    def updateFromEstimate(self, estimate, alpha):
        # estimate = estimateFromMask return value
        # 该值每次迭代都没有变化，照道理讲每个loop estimate出来的东西不可能是一样的（mus变了就会变=。=）
        # print(estimate)
        # 此处可以理解类似梯度下降，在调参，为啥写质心不是很清楚，怎么还用到了Kmeans的概念？
        Dmus = estimate - self.mus
        # self.mus = self.mus
        self.mus = self.mus + alpha * (Dmus)

    def compute_optimal_transport(self, M, r, c, epsilon=1e-6):

        r = r.cuda()
        c = c.cuda()
        n_runs, n, m = M.shape

        # P=M
        # 论文全篇都没提到以e为底的函数，哪来的我靠？
        # print(M)
        P = torch.exp(- self.lamda * M)
        # print(P)
        P /= P.view((n_runs, -1)).sum(1).unsqueeze(1).unsqueeze(1)
        # print(P)

        u = torch.zeros(n_runs, n).cuda()
        maxiters = 1000
        iters = 1
        # normalize this matrix
        while torch.max(torch.abs(u - P.sum(2))) > epsilon:
            u = P.sum(2)
            P *= (r / u).view((n_runs, -1, 1))
            P *= (c / P.sum(1)).view((n_runs, 1, -1))
            if iters == maxiters:
                break
            iters = iters + 1
        return P, torch.sum(P * M)

    def getProbas(self):
        # compute squared dist to centroids [n_runs][n_samples][n_ways]
        # 到质心的欧式距离平方，why？
        dist = (ndatas.unsqueeze(2) - self.mus.unsqueeze(1)).norm(dim=3).pow(2)

        p_xj = torch.zeros_like(dist)
        r = torch.ones(n_runs, n_usamples)
        c = torch.ones(n_runs, n_ways) * n_queries

        p_xj_test, _ = self.compute_optimal_transport(dist[:, n_lsamples:], r, c, epsilon=1e-6)
        p_xj[:, n_lsamples:] = p_xj_test

        p_xj[:, :n_lsamples].fill_(0)
        p_xj[:, :n_lsamples].scatter_(2, labels[:, :n_lsamples].unsqueeze(2), 1)

        return p_xj

    def estimateFromMask(self, mask):

        emus = mask.permute(0, 2, 1).matmul(ndatas).div(mask.sum(dim=1).unsqueeze(2))

        return emus


# =========================================
#    MAP
# =========================================

class MAP:
    def __init__(self, alpha=None):

        self.verbose = False
        self.progressBar = False
        self.alpha = alpha

    def getAccuracy(self, probas):
        olabels = probas.argmax(dim=2)
        matches = labels.eq(olabels).float()
        acc_test = matches[:, n_lsamples:].mean(1)

        m = acc_test.mean().item()
        pm = acc_test.std().item() * 1.96 / math.sqrt(n_runs)
        return m, pm

    def performEpoch(self, model, epochInfo=None):

        p_xj = model.getProbas()
        self.probas = p_xj

        if self.verbose:
            print("accuracy from filtered probas", self.getAccuracy(self.probas))

        m_estimates = model.estimateFromMask(self.probas)

        # update centroids
        model.updateFromEstimate(m_estimates, self.alpha)

        if self.verbose:
            # 此处看似没传参，其实内部的mus随上面的代码一直在更新，所以ACC会变化，理论上应该是越来越准确
            op_xj = model.getProbas()
            acc = self.getAccuracy(op_xj)
            print("output model accuracy", acc)

    def loop(self, model, n_epochs=20):

        self.probas = model.getProbas()
        if self.verbose:
            print("initialisation model accuracy", self.getAccuracy(self.probas))

        if self.progressBar:
            if type(self.progressBar) == bool:
                pb = tqdm(total=n_epochs, disable=False)
            else:
                pb = self.progressBar

        for epoch in range(1, n_epochs + 1):
            if self.verbose:
                print("----- epoch[{:3d}]  lr_p: {:0.3f}  lr_m: {:0.3f}".format(epoch, self.alpha, 0))
            self.performEpoch(model, epochInfo=(epoch, n_epochs))
            if (self.progressBar): pb.update()

        # get final accuracy and return it
        op_xj = model.getProbas()
        acc = self.getAccuracy(op_xj)
        return acc


if __name__ == '__main__':
    # ---- data loading
    n_ways = 5
    n_shot = 5
    n_queries = 15
    n_runs = 10000
    # 25种labeled，75种unlabeled
    n_lsamples = n_ways * n_shot
    n_usamples = n_ways * n_queries
    n_samples = n_lsamples + n_usamples

    import FSLTask

    cfg = {'shot': n_shot, 'ways': n_ways, 'queries': n_queries}
    FSLTask.loadDataSet("cifar")
    FSLTask.setRandomStates(cfg)
    # test时，随机生成1万个episode，20次epoch(1次epoch包含多个episode或task)，每个episode有way-shot用于support，剩下的用于query，至少理论上是这样的
    ndatas = FSLTask.GenerateRunSet(cfg=cfg)
    print('GenerateRunSet...')
    print(ndatas.shape)
    # 5way5shot15query时，torch.Size([10000, 5, 20, 640])，5way，每way5个做support，15个做query，1次episode总数100张图，至少理论上是这样的
    # 5way1shot5query时，torch.Size([10000, 5, 6, 640])
    # 维度换位，不懂为啥
    ndatas = ndatas.permute(0, 2, 1, 3).reshape(n_runs, n_samples, -1)
    print('ndatas.permute...')
    print(ndatas.shape)
    # 5way5shot15query时，torch.Size([10000, 100, 640])
    # 5way1shot5query时，torch.Size([10000, 30, 640])
    labels = torch.arange(n_ways).view(1, 1, n_ways).expand(n_runs, n_shot + n_queries, 5).clone().view(n_runs,
                                                                                                        n_samples)
    print(labels.shape)
    # 5way5shot15query时，torch.Size([10000, 100])
    # 5way1shot5query时，torch.Size([10000, 30])

    # 仅禁用这两行所谓的幂变换准确率基本不降，匪夷所思。。。，文中PT公式提到的epsilon和beta全在这行里，注释与否效果居然几乎一样？？
    # # Power transform
    # beta = 0.5
    # ndatas[:,] = torch.pow(ndatas[:,]+1e-6, beta)

    # QR分解类似SVD分解，本文取了thin版的上三角矩阵R，目的是降噪压缩，这行如果不加的话显卡内存可能不够，和PT MAP没关联，仅仅是图像预处理
    ndatas = QRreduction(ndatas)

    # 下面2行保留其一，准确率都几乎不降

    # 求L2范数，然后L2范数归一化，本文目测只是为了加快收敛
    ndatas = scaleEachUnitaryDatas(ndatas)

    # 图像去均值，防止过拟合，对准确率只是有提升效果，本文提升不到1%
    # trans-mean-sub
    # ndatas = centerDatas(ndatas)

    # 禁用幂变换后准确率直降到20%+，而且以上所谓的幂变换过程在2060super+i5的配置下占用率也会飙到100%
    print("size of the datas...", ndatas.size())

    # switch to cuda
    ndatas = ndatas.cuda()
    labels = labels.cuda()

    # MAP
    # lamda部分不禁用的话+L2正则化，准确率已经达到89%，和论文结果其实不太一样
    lamda = 10
    model = GaussianModel(n_ways, lamda)
    # 这个model完全没有backbone，也没有从checkpoints加载模型，到底是个啥鬼=。=纯统计学模型？？？
    # 文中所指的MAP 最大后验概率估计，其对数据的分布已经有了一个先验分布，而这个理念本身就有一定问题，因为当样本极其少的时候(1-shot)，怎么准确的得出先验分布呢
    # 当然，MAP这种方法用在其他场景是不错的，包括幂变换本身也是，是修正数据分布的好方法
    # 总体而言，本文的方法只是针对个别特定的数据集作出的优化，而非里程碑式的进步
    n_nfeat = ndatas.size(2)
    # n_nfeat = 100
    print(n_nfeat)
    model.initFromLabelledDatas()

    # alpha只是一个meta task的学习率，影响不大
    alpha = 0.2
    optim = MAP(alpha)

    # 只禁用MAP后准确率直降到0.22%
    # PT&MAP全禁用后准确率直降到0.2%
    optim.verbose = True
    optim.progressBar = True

    acc_test = optim.loop(model, n_epochs=20)

    print("final accuracy found {:0.2f} +- {:0.2f}".format(*(100 * x for x in acc_test)))
